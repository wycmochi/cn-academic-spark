...
Reads OOXML directly via `scripts/pptx_to_svg/` and writes a reusable reference workspace:

- `manifest.json` — single source of truth for slide size, theme colors, fonts,
  master/layout placeholders, protected regions, page-number slots, overlap
  audits, layout profiles, asset inventory, and per-slide / per-layout /
  per-master metadata
...
- `svg/` — primary view: by default the layered template view (every master
  and layout in the deck rendered once each as locked references named
  `master_*.svg` / `layout_*.svg`; `slide_NN.svg` contains normal-page
  slide-local examples, while manifest layout profiles define the fillable
  slots and protected regions, and an `inheritance.json` describes the reuse graph)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "renderings (template designers see master/layout placeholders "
            "as fill contracts, protected regions as no-overlap zones, and "
            "slide_NN as usage examples, plus svg/inheritance.json) and svg-flat/ holds "
