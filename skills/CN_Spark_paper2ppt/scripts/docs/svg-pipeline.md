# SVG Pipeline Tools
document explanation(It doesn't affect the process, it only helps with understanding）：本文件在需要了解 SVG 到 PPTX 流水线时读取；它说明 SVG 后处理、质检和转换命令。
...
It aggregates:
- `clean_placeholder_prompts.py`: remove unused PPT placeholder prompts / guide boxes before export.
- `embed_icons.py`
- `crop_images.py`
- `fix_image_aspect.py`
- `embed_images.py`
...
- Speaker notes are exported as a standalone DOCX. PPTX notes are stripped from every generated PPTX because notesSlide / notesMaster markup can trigger PowerPoint repair or COM/RPC open failures.
- Recorded narration is opt-in:
  ...
  - `--recorded-narration audio` embeds each matching audio file and writes slide auto-advance timings from audio duration. Speaker notes remain in the standalone DOCX and are not embedded into the PPTX.
