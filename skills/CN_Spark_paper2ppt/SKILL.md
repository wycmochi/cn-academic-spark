...
- Academic decks require internal TechnicalRoute A/B by default. Generated project files may not opt out with `technicalroute_required: false`, `technical_route_required: false`, `skip_technicalroute: true`, or similar wording. Create `<project_path>/technicalroute/route_01/` during planning; a missing `technicalroute/` folder is a blocking failure, not an optional omission.
...
- Language policy is Chinese-first. If the user input/source package is Chinese, every PPT text object must be Chinese. If the source paper is English, the cover must show a Chinese translated paper title plus the original English title as secondary/original-title metadata; all non-cover generated PPT text must be Chinese except source-figure pixels, formula variables, DOI/citations, journal names, author names, and explicitly marked original-title metadata. `svg_quality_checker.py` blocks English-dominant generated titles/body text.
...
Blocking confirmation: ask once for canvas, template choice, page count, audience, style goal, palette, icon style, typography, and image policy. Page count means editable/report-content PPT pages; the TechnicalRoute Version B AI reference image page is extra and does not count against the requested page target. Record the user's number as `user_requested_page_count` and `requested_regular_page_count` in `design_spec.md` / `spec_lock.md`; if the user asks for 18 pages, build 18 regular editable/report pages and export 19 total slides after the Version B direct image page is inserted.
...
Gate: academic decks require this step by default. Generated `design_spec.md`, `spec_lock.md`, outline, notes, SVG pages, or QA text may not bypass it with `technicalroute_required: false`, `technical_route_required: false`, `skip_technicalroute: true`, or equivalent wording. Any resource or page that requires `technical_route`, `research_framework`, `thinking_map`, `whole_paper_workflow`, `concept_framework`, mechanism diagram, principle diagram, or `embed_technicalroute: true` also counts as this gate. A page roster item, slide title, notes line, or final QA item named `Technical Route`, `Research Workflow`, `Workflow`, `Pipeline`, `技术路线`, `技术路线页`, `全文方法链条`, or equivalent Chinese wording counts as this gate; a locally hand-drawn workflow page is not a substitute for the required A/B TechnicalRoute output. `svg_quality_checker.py` blocks export when an academic project has no TechnicalRoute A/B output, even if the agent forgot to declare the route page in `design_spec.md`.
...
Page-count rule: Version B is an extra reference page, not part of the user's requested slide count. If the user asks for 18 PPT pages, generate exactly 18 regular editable/report-content pages, then insert the direct Version B AI reference page through `_direct_image_slides.json`, so the final PPTX must contain 19 slides. Report this clearly in `ppt_outline_cn.md` / final QA as “AI 技术路线参考图页，不计入用户页数指标；用户要求 18 页时最终导出 19 页”. The manifest entry must include `page_count_policy: extra_reference_page_not_counted`, `counts_against_user_page_count: false`, and `page_count_delta: 1`.

Execution order:
1. Create `<project_path>/technicalroute/route_01/` immediately when planning the deck, then read `references/technicalroute/diagram-contract.md` and create `<route_workdir>/contract.md` with `generate_route_image.py contract`. Ask the user only for blocking ambiguity or explicit pre-approval requests; otherwise record conservative assumptions and continue.
...
route_ai_page_count_policy: extra_reference_page_not_counted
user_requested_page_count: <regular editable/report-content page count>
final_pptx_page_count: <user_requested_page_count + number_of_route_ai_direct_picture_pages>
